...
export default function MediaUpload() {
  ...
  const handleDetectColors = async () => {
    ...
    try {
      ...
      const response = await fetch(SERVER_URL, {
        ...
        body: JSON.stringify({ image: base64, cvd_mode: colorBlindMode }),
      });
  ...
  const handleColorDetected = (color: DetectedColor) => {
    setDetectedColor(color);
    setColorPanelVisible(true);
  };

  const handleSaveColor = (color: DetectedColor) => {
    const exists = savedColors.some((c) => c.hex === color.hex);
    if (!exists) {
