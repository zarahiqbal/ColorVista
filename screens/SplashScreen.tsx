...
// Where authenticated users should land. Adjust to your actual route.
const HOME_ROUTE = "/dashboard";
const LOGIN_ROUTE = "../auth/login";

// Minimum time the splash stays visible (so the animation isn't cut short).
...
const SplashScreen: React.FC = () => {
  ...
  // Tracks whether the splash has shown for its minimum duration.
  ...
  useEffect(() => {
    ...
    // Mark the minimum splash time as elapsed (navigation happens in the
    // effect below, once auth state is also resolved).
    const timer = setTimeout(() => setMinTimeElapsed(true), MIN_SPLASH_MS);

    return () => clearTimeout(timer);
  }, []);

  // Navigate once auth state is resolved AND the splash has shown its minimum
  // time. Authenticated (or guest) users go home; everyone else goes to login.
  useEffect(() => {
    if (navigatedRef.current) return;
    if (!minTimeElapsed || isLoading) return;

    // A guest is NOT treated as a logged-in user here, so guests go to login.
    // Remove `&& !user.isGuest` if you want guests to reach the dashboard too.
    const isAuthenticated = !!user && !user.isGuest;

    // Temporary: confirm what the splash sees. Remove once verified.
    console.log("[Splash] user:", user, "isAuthenticated:", isAuthenticated);

    navigatedRef.current = true;
    router.replace((isAuthenticated ? HOME_ROUTE : LOGIN_ROUTE) as Href);
  }, [minTimeElapsed, isLoading, user, router]);
